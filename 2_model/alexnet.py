...
class alexnet:
    ...
    def convlayers(self):
        ...
        with tf.name_scope('conv1') as scope:
            kernel = tf.get_variable(name='conv1_W', shape=[11, 11, 3, 96], 
        ...
        with tf.name_scope('conv4') as scope:
            kernel = tf.get_variable(name='conv4_W', shape=[3, 3, 384, 384], 
                initializer=tf.random_normal_initializer(stddev=0.01),
                regularizer=tf.contrib.layers.l2_regularizer(0.0005))

            conv = tf.nn.conv2d(self.conv3, kernel, [1, 1, 1, 1], padding='SAME')
            biases = tf.Variable(tf.constant(0.0, shape=[384], dtype=tf.float32),
                                 trainable=True, name='conv4_b')
            out = tf.nn.bias_add(conv, biases)
